...
from deep_face_try_code import detect_emotions
...
def detect_emotions(result_list):
    ...
    while True:
        ret,frame = cap.read()
        result = DeepFace.analyze(img_path = frame , actions=['emotion'], enforce_detection=False ) # pre-trained model to analyze the emotions
    ...
    text = emotions_results[-1] # extracting the last detected emotions
    ...
    result_list.append(emotion)
    detected_emotion_label.config(text="Detected emotion: "+ emotion)
    play_audio(emotion) # play audio based on the detected emotions
    ...
    with open(emotion_detection_path, 'a') as f:
        ...
        f.write(f"Detected Emotions: {emotion}\n")
...
school_logo_image = Image.open("/home/akshay/Desktop/practise/photos/school/school_logo.png")
school_logo_image = school_logo_image.resize((200, 200), Image.ANTIALIAS)
school_logo_photo = ImageTk.PhotoImage(school_logo_image)

jcu_logo_image = Image.open("/home/akshay/Desktop/practise/photos/JCU/jcu_logo.png")
